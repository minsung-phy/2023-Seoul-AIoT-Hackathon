...
class MenuScreen:
    ...
    def open_decibel_monitoring(self):
        top = Toplevel(self.master)  # Toplevel widget를 사용하여 새 창을 생성합니다.
        app = DecibelApp(top)
...
class DecibelApp:
    def __init__(self, master, bridge_name=None):
        self.master = master
        self.master.title(f"Decibel Monitoring - {bridge_name}" if bridge_name else "Decibel Monitoring")
    ...
    def setup_menu(self):
        ...
        self.han_river_menu = Menu(self.menu, tearoff=0)
        self.menu.add_cascade(label="한강 데시벨", menu=self.han_river_menu)
        
        bridges = ["동작대교", "동호대교", "잠실대교", "청담대교", "가양대교", "성산대교", "양화대교",
                   "서강대교", "월드컵대교", "원효대교", "마포대교", "한강대교", "반포대교(잠수교)",
                   "한남대교", "성수대교", "영동대교", "잠실대교", "올림픽대교", "천호대교", "광진교"]
        
        for bridge in bridges:
            self.han_river_menu.add_command(label=bridge, command=lambda b=bridge: self.show_bridge_decibel(b))

    def show_bridge_decibel(self, bridge_name):
        new_window = tk.Toplevel(self.master)
        DecibelApp(new_window, bridge_name)
